...
if __name__ == '__main__':
    ...
    train_data = [data[i] for i in range(len(data)) if int(i) != sample_ind]
    test_data = [data[i] for i in range(len(data)) if int(i) == sample_ind]

    c1 = NaiveBayes(get_words)
    c2 = NaiveBayes(get_words_emoticons)

    for item in train_data:
        c1.train(item[1], item[0])
        c2.train(item[1], item[0])

    class_wo_emoticon = c1.classify_document(test_data[0][1])
    class_emoticon = c2.classify_document(test_data[0][1])

    print(test_data[0][1])
    print(f"Vistinska klasa: {test_data[0][0]}")
    print(f"Klasa predvidena so Naiven Baes (bez emotikoni): {class_wo_emoticon}")
    print(f"Klasa predvidena so Naiven Baes (so emotikoni): {class_emoticon}")