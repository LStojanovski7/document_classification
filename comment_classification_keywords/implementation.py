...
def get_words_custom2(doc, words_to_include=words_to_include):
    words = set()
    for word in re.split('\\W+', doc):
...
if __name__ == '__main__':
    ...
    c2 = NaiveBayes(get_words_custom2)

    for item in data:
        c1.train(item[0], item[1])
        c2.train(item[0], item[1])

    print(f"Klasa predvidena so site zborovi: {c1.classify_document(comment)}")
    print(f"Klasa predvidena so samo kluchni zborovi: {c2.classify_document(comment)}")