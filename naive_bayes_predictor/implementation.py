...
if __name__ == '__main__':
    ...
    divide = (len(train_data) * 40) // 100

    cl1 = NaiveBayes(get_words)
    cl2 = NaiveBayes(get_words)

    train_data1 = train_data[:divide]
    train_data2 = train_data[divide:divide + divide]

    test_data = train_data[divide + divide:]

    for item in train_data1:
        cl1.train(item[0], item[1])

    for item in train_data2:
        cl2.train(item[0], item[1])
    ...
    for item in test_data:
        cls1 = cl1.classify_document(item[0])
        cls2 = cl2.classify_document(item[0])

        if cls1 == cls2:
            if cls1 == item[1]:
                hits += 1

    cls1 = cl1.classify_document(sample)
    cls2 = cl2.classify_document(sample)
